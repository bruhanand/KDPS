...
Three things a live operator owns are deliberately left alone:
...
- **Whether a role is active.** A role two administrators switched off stays off; the seed only sets this when it creates the role.

There is one exception to "never changes a cell", and it only ever narrows.
The four money floor rules are locked in code, so a release can tighten one.
When that happens the next seed pulls any stored cell that now sits above the floor down to it, and prints which role and which cell it moved.
