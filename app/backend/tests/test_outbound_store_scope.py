...
def scope_scaffold(db):
    ...
    gstin_jh = Gstin.objects.create(
        gstin="20AAACS1234B1ZQ", state_code="20", state_name="Jharkhand",
        legal_entity=entity,
    )
    gstin_bh = Gstin.objects.create(
        gstin="10AAACS1234B1ZQ", state_code="10", state_name="Bihar",
        legal_entity=entity,
    )
    ...
    sm_user = User.objects.create_user(
        username="scope_sm", password="Test@123",
        role=sm_role, entity=entity, scope_type=ScopeType.STORE,
    )
    ...
    admin_user = User.objects.create_user(
        username="scope_admin", password="Test@123",
        role=admin_role, entity=entity, scope_type=ScopeType.ALL,
    )

    # Seed stock at both stores
    for store, gstin in [(store_deo, gstin_jh), (store_banka, gstin_bh)]:
        StockOnHand.objects.create(
            store=store, gstin=gstin, sku_code="SC-SKU1",
            design="Test", color="Red", size="M", brand="ScopeOwned",
            season="SS26", item="shirt", hsn="6205",
            net_qty=20, net_value_paise=20 * 10000,
        )
        StockLedgerEntry.objects.create(
            store=store, gstin=gstin, sku_code="SC-SKU1",
            design="Test", color="Red", size="M", brand="ScopeOwned",
            season="SS26", item="shirt", hsn="6205",
            qty=20, amount=20 * 10000, kind="pt_inward",
            doc_number="SC-SEED", line_no=1,
        )
        # SOR stock for V-flip tests
        StockOnHand.objects.create(
            store=store, gstin=gstin, sku_code="SC-SKU2",
            design="Tee", color="White", size="L", brand="ScopeSOR",
            season="SS26", item="tee", hsn="6106",
            net_qty=10, net_value_paise=10 * 5000,
        )
        StockLedgerEntry.objects.create(
            store=store, gstin=gstin, sku_code="SC-SKU2",
            design="Tee", color="White", size="L", brand="ScopeSOR",
            season="SS26", item="tee", hsn="6106",
            qty=10, amount=10 * 5000, kind="pt_inward",
            doc_number="SC-SEED2", line_no=1,
        )
    ...
    for code in ["SC-DEO", "SC-BNK"]:
        for dt in ["STO", "RTV", "ADJ", "WRO", "VFL"]:
            VoucherSeries.objects.create(
                fy=fy, store_code=code, doc_type=dt,
                prefix=f"{code}/{dt}/{fy}/", next_seq=1,
            )
...
def test_sm_cannot_submit_rtv_outside_scope(scope_scaffold):
    ...
    rtv = ReturnToVendor.objects.create(
        store=s["store_banka"], vendor=s["vendor"], brand=s["brand_owned"],
        return_type="defective", created_by=s["admin_user"],
    )
    ReturnToVendorLine.objects.create(
        rtv=rtv, sku_code="SC-SKU1", qty=1, unit_cost_paise=10000,
    )
    c = _client(s["sm_user"])
...
def test_sm_cannot_create_transfer_from_outside_scope(scope_scaffold):
    ...
    s = scope_scaffold
    c = _client(s["sm_user"])
    resp = c.post("/api/outbound/transfers", {
...
def test_sm_cannot_receive_transfer_at_outside_store(scope_scaffold):
    ...
    transfer = StoreTransfer.objects.create(
        source_store=s["store_deo"], destination_store=s["store_banka"],
        transfer_type="inter_store", created_by=s["admin_user"],
    )
    StoreTransferLine.objects.create(
        transfer=transfer, sku_code="SC-SKU1", qty_dispatched=1,
        unit_cost_paise=10000,
    )
    post_transfer_dispatch(transfer, user=s["admin_user"])

    c = _client(s["sm_user"])
    resp = c.post(f"/api/outbound/transfers/{transfer.id}/receive", {
        "received_quantities": {},
    }, format="json")
    assert resp.status_code == 403
...
def test_sm_cannot_create_adjustment_outside_scope(scope_scaffold):
    ...
    s = scope_scaffold
    c = _client(s["sm_user"])
    resp = c.post("/api/outbound/adjustments", {
        "store": s["store_banka"].id,
        "reason": "miscount",
        "lines": [{"sku_code": "SC-SKU1", "book_qty": 20, "counted_qty": 19,
                    "adj_qty": -1, "unit_cost_paise": 10000}],
    }, format="json")
    assert resp.status_code == 403
...
def test_admin_cannot_writeoff_outside_assigned_stores(scope_scaffold):
    ...
    scoped_admin = User.objects.create_user(
        username="scope_owner", password="Test@123",
        role=Role.objects.get(code="owner"), entity=s["entity"],
        scope_type=ScopeType.STORE,
    )
    scoped_admin.stores.add(s["store_deo"])

    c = _client(scoped_admin)
    resp = c.post("/api/outbound/writeoffs", {
        "store": s["store_banka"].id,
        "reason": "dead_stock",
...
def test_admin_can_vflip_any_store(scope_scaffold):
    ...
    s = scope_scaffold
    c = _client(s["admin_user"])
    resp = c.post("/api/outbound/vflips", {
        "store": s["store_banka"].id,
