// forked from design-sync lib/source-kit.mjs — KDPS is an app, not a library: its
// synth entry must skip the app bootstrap (main.tsx mounts into #root and throws,
// aborting the IIFE before window.KDPS is assigned).
...
// App-bootstrap modules have import-time side effects (ReactDOM.createRoot on
// #root). Bundled into the synth entry they throw before the IIFE publishes
// window.<GLOBAL>, so every export goes missing. Components only.
const BOOTSTRAP_RX = /(?:^|\/)main\.(?:tsx|jsx)$/;
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !BOOTSTRAP_RX.test(p));
