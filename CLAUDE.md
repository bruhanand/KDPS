...
Shard balance comes from `app/backend/.test_durations`. Regenerate it when the suite's shape changes materially: `uv run pytest tests --store-durations`. One hole is still open on #292: mypy covers only `core config`.
