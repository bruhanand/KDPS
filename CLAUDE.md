...
- Repo-level commands exist now. `npm run ci` (ruff · mypy strict · migration check · import-linter · pytest · tsc) is the **local acceptance gate**, and `.github/workflows/ci.yml` runs that same gate in the cloud on push - not a subset of it (#292). `docker-compose` gives a local Postgres and pre-commit hooks run ruff/mypy. See `README.md` (run) and `DEPLOY.md` (Render).

Cloud CI (`.github/workflows/ci.yml`) triggers on **push only** - no `pull_request` (the push run already covers the PR head) and, deliberately, **no `paths-ignore` on the trigger**: a run a path filter suppresses reports no check at all, which is indistinguishable from a run GitHub silently dropped and makes the workflow useless as a required check (#301). The workflow therefore always starts, and filtering happens per job. Jobs: `changes` (which trees moved), `lint` (ruff format · ruff check · `mypy core config` · import-linter), `backend-kernel` (kernel anti-cheat + migration-drift guards), `backend-suite` (**8 parallel shards** via `pytest-split`, each with its own Postgres + seed + uvicorn), `frontend` (build + tsc + vitest), and `ci` - an always-run job that aggregates the rest and is **the single required check on `main`**. Typical full run ~4-5 min; a push touching neither tree finishes green in well under a minute.
