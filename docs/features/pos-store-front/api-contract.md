...
7. `live.in_transit[].expected` is **as built** the transfer's `expected_arrival_note` (free text: "Bus, Friday evening"), not the date this sketch showed. No expected-arrival date column exists on a transfer, and deriving one from the note would be inventing it. If a real date is wanted, it is a column on `StoreTransfer` first.
