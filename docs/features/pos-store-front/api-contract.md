...
- **Two sections were added while building the Billing screen (#181): `seasons` and `policy`.**
  ...
  A2 says a scan that does not name a season resolves to the *oldest live* one with stock, and `sell.services.resolve` makes that choice by ranking `(is_closed, sort_order)` from that master - so a till without it would fall back to sorting names, and "FW25 before SS26" is true only by the accident of the alphabet.
  The season the till picks is the season it writes on the line, and the accept pipeline honours an exact `(barcode, season)` outright, so the till's choice is the one that reaches the books.
